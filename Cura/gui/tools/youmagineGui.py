...
class youmagineManager(object):
	...
	def checkAuthorizationThread(self):
		wx.CallAfter(self._indicatorWindow.showBusy, _("Loading"))
...
class workingIndicatorWindow(wx.Frame):
	def __init__(self, parent):
		super(workingIndicatorWindow, self).__init__(parent, title='KAIDEA', style=wx.FRAME_TOOL_WINDOW|wx.FRAME_FLOAT_ON_PARENT|wx.FRAME_NO_TASKBAR|wx.CAPTION)
...
class getAuthorizationWindow(wx.Frame):
	def __init__(self, parent, ym):
		super(getAuthorizationWindow, self).__init__(parent, title='KAIDEA')
		self._panel = wx.Panel(self)
		self.SetSizer(wx.BoxSizer())
		self.GetSizer().Add(self._panel, 1, wx.EXPAND)
		self._ym = ym
		...
		self._requestButton = wx.Button(self._panel, -1, _("Request authorization from KAIDEA"))
...
class newDesignWindow(wx.Frame):
	def __init__(self, parent, manager, ym):
		super(newDesignWindow, self).__init__(parent, title='Share on KAIDEA')
		p = wx.Panel(self)
		self.SetSizer(wx.BoxSizer())
...
class webcamPhotoWindow(wx.Frame):
	def __init__(self, parent):
		super(webcamPhotoWindow, self).__init__(parent, title='KAIDEA')
		p = wx.Panel(self)
		self.panel = p
		self.SetSizer(wx.BoxSizer())
		self.GetSizer().Add(p, 1, wx.EXPAND)
