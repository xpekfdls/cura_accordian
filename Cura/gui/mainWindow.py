...
from Cura.gui import loginPanel
from Cura.gui import preferencesDialog
from Cura.gui import configWizard
from Cura.gui import neuronPanel
...
class mainWindow(wx.Frame):
	def __init__(self):
		...
		toolsMenu.AppendSeparator()
		i = toolsMenu.Append(-1, _(u"stl 리스트 숨기기."), kind=wx.ITEM_RADIO)
		self.switchToStlHide = i
		self.Bind(wx.EVT_MENU, self.OnSimpleSwitch_right, i)
		
		i = toolsMenu.Append(-1, _(u"stl 리스트 보이기"), kind=wx.ITEM_RADIO)
		self.switchToStlShow = i
		self.Bind(wx.EVT_MENU, self.OnNormalSwitch_right, i)	
		...
		i = helpMenu.Append(-1, _(u"KAIDEA website 열기..."))
		self.Bind(wx.EVT_MENU, lambda e: webbrowser.open('https://www.kaidea.co.kr/'), i)
		...
		self.vsplitter = wx.SplitterWindow(self.splitter, style = wx.SP_3D | wx.SP_LIVE_UPDATE)
		self.leftPane = wx.Panel(self.splitter, style=wx.BORDER_NONE)
		self.rightPane = wx.Panel(self.vsplitter, style=wx.BORDER_NONE)
		#self.rightPane = wx.Panel(self.splitter, style=wx.BORDER_NONE)
		self.neuronPane = wx.Panel(self.vsplitter, style=wx.BORDER_NONE)
		...
		self.neuronPanel = neuronPanel(self.neuronPane, lambda : self.scene.sceneUpdated())
		self.neuronsimplePanel = simpleMode.simpleModePanel(self.neuronPane, lambda : self.scene.sceneUpdated())
		...
		self.neuronSizer = wx.BoxSizer(wx.VERTICAL)
		self.neuronSizer.Add(self.neuronPanel, 1, wx.EXPAND)
		self.neuronSizer.Add(self.neuronsimplePanel)
		self.neuronPane.SetSizer(self.neuronSizer)
		...
		self.simpleSettingsPanel.Show(True)
		self.normalSettingsPanel.Show(True)
		self.neuronPanel.Show(True)
		self.neuronsimplePanel.Show(True)
		...
		try:
			...
			print(self.normalSashPos)
			self.showSashPos = int(profile.getPreference('window_show_sash'))
			#print(self.showSashPos)
		...
		if self.showSashPos < self.normalSettingsPanel.printPanel.GetBestSize()[0] + 5:
			width = int(profile.getPreference('window_width'))
			self.showSashPos = width-self.normalSettingsPanel.printPanel.GetBestSize()[0] + 300 - self.normalSashPos	
			print("checkcheck")
		print(self.normalSashPos)	
		"""
		try:
			...
			self.Maximize(True)
		if self.showSashPos < self.normalSettingsPanel.printPanel.GetBestSize()[0] + 5:
			self.showSashPos = self.normalSettingsPanel.printPanel.GetBestSize()[0] + 5	
			"""
		# #########################################################################################################
			

                self.vsplitter.SplitVertically(self.rightPane, self.neuronPane, self.showSashPos)
		self.splitter.SplitVertically(self.leftPane, self.vsplitter,  self.normalSashPos)
		...
		self.updateSliceMode_right()
		self.scene.SetFocus()
		#print(self.showSashPos)
		
		#sceneView.youMagineButton
		#self.youMagineButton = openglGui.glButton(self, 26, _("Join KAIDEA"), (2,0), lambda button: youmagineGui.youmagineManager(self.GetTopLevelParent(), self._scene))
	...
	def updateSliceMode_right(self):
		isHide = profile.getPreference('startMode_right') == 'Hide'
		
		self.neuronPanel.Show(not isHide)
		self.neuronsimplePanel.Show(isHide)
		self.neuronPane.Layout()
	
		for i in self.normalModeOnlyItems:
			i.Enable(not isHide)
		if isHide:
			self.switchToStlHide.Check()
		else:
			self.switchToStlShow.Check()
		...
		if isHide:
			...
			width = int(profile.getPreference('window_width'))
			self.showSashPos = width-self.normalSashPos
			self.showSashPos = self.vsplitter.GetSashPosition()

			# Change location of sash to width of quick mode pane
			#(width, height) = self.neuronsimplePanel.GetSizer().GetSize()
			self.vsplitter.SetSashPosition(width, True)

			# Disable sash
			self.vsplitter.SetSashSize(0)
		else:
			self.vsplitter.SetSashPosition(self.showSashPos, True)
			# Enabled sash
			self.vsplitter.SetSashSize(4)
		self.defaultFirmwareInstallMenuItem.Enable(firmwareInstall.getDefaultFirmware() is not None)
		if profile.getMachineSetting('machine_type') == 'ultimaker2':
			self.bedLevelWizardMenuItem.Enable(False)
			self.headOffsetWizardMenuItem.Enable(False)
	...
	def updateProfileToAllControls_right(self):
		self.scene.updateProfileToControls()
		self.neuronPanel.updateProfileToControls()
		self.neuronsimplePanel.updateProfileToControls()	
	...
	def reloadSettingPanels_right(self):
		self.neuronSizer.Detach(self.neuronPanel)
		self.neuronSizer.Detach(self.neuronsimplePanel)
		self.neuronPanel.Destroy()
		self.neuronsimplePanel.Destroy()
		self.neuronsimplePanel = simpleMode.simpleModePanel(self.neuronPane, lambda : self.scene.sceneUpdated())
		self.neuronPanel = neuronPanel(self.neuronePane, lambda : self.scene.sceneUpdated())
		self.neuronSizer.Add(self.neuronsimplePanel, 1)
		self.neuronSizerSizer.Add(self.neuronPanel, 1, wx.EXPAND)
		#self.updateSliceMode()
		self.updateSliceMode_right()
		self.updateProfileToAllControls_right()
	...
	def updateMachineMenu(self):
		...
		i = self.machineMenu.Append(-1, _(u"기기 설정..."))
		self.Bind(wx.EVT_MENU, self.OnMachineSettings, i)
		...
		i = self.machineMenu.Append(-1, _(u"커스텀 펌웨어 인스톨..."))
		self.Bind(wx.EVT_MENU, self.OnCustomFirmware, i)

	def OnLoadProfile(self, e):
		dlg=wx.FileDialog(self, _(u"선택된 설정파일 불러오기"), os.path.split(profile.getPreference('lastFile'))[0], style=wx.FD_OPEN|wx.FD_FILE_MUST_EXIST)
	...
	def OnSimpleSwitch_right(self, e):
		profile.putPreference('startMode_right', 'Hide')
		self.updateSliceMode_right()

	def OnNormalSwitch_right(self, e):
		profile.putPreference('startMode_right', 'Show')
		self.updateSliceMode_right()		
	...
	def OnFirstRunWizard(self, e):
		...
		self.reloadSettingPanels_right()
	...
	def OnClose(self, e):
		...
		if not self.IsMaximized() and not self.IsIconized():
			...
			profile.putPreference('window_show_sash', self.showSashPos)
...
class normalSettingsPanel(configBase.configPanelBase):
	...
	def __init__(self, parent, callback = None):
		...
		(left, right, self.printPanel) = self.CreateDynamicConfigTab(self.nb, u'기본')
		self._addSettingsToPanels('basic', left, right)
		self.SizeLabelWidths(left, right)

		(left, right, self.advancedPanel) = self.CreateDynamicConfigTab(self.nb, u'고급')
		self._addSettingsToPanels('advanced', left, right)
		self.SizeLabelWidths(left, right)

		#Plugin page
		self.pluginPanel = pluginPanel.pluginPanel(self.nb, callback)
		self.nb.AddPage(self.pluginPanel, _(u"플러그인"))
		...
		if profile.getMachineSetting('gcode_flavor') == 'UltiGCode':
			...
			self.nb.AddPage(self.alterationPanel, u"시작/종료-GCode")
...
class neuronPanel(configBase.configPanelBase):
	"Main user interface window"
	def __init__(self, parent, callback = None):
		super(neuronPanel, self).__init__(parent, callback)
		...
		#Plugin page
		self.pluginPanel = loginPanel.loginPanel(self.nb, callback)
		self.nb.AddPage(self.pluginPanel, _(u"플러그인"))
	...
	def updateProfileToControls(self):
		super(neuronPanel, self).updateProfileToControls()
		if self.alterationPanel is not None:
			self.alterationPanel.updateProfileToControls()
		self.pluginPanel.updateProfileToControls()		
