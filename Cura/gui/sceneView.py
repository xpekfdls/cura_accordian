...
class SceneView(openglGui.glGuiPanel):
	def __init__(self, parent):
		...
		#self.youMagineButton = openglGui.glButton(self, 26, _("Join KAIDEA"), (2,0), self.viewlogin)
		...
		self.QualityButton = openglGui.glRadioButton(self, 5, _("Quality Setting"), (3,-1), group, self.OnToolSelect)
                self.highQualityButton = openglGui.glButton(self, 2, _("High Quality"), (3,-4),   self.OnHighQuality, texture = 2)
		self.normalQualityButton = openglGui.glButton(self, 3, _("Normal Quality"), (3,-3),   self.OnNormalQuality, texture = 2)
		self.lowQualityButton = openglGui.glButton(self, 4, _("Low Quality"), (3,-2),   self.OnLowQuality, texture = 2)
		...
		self.mirrorZButton       = openglGui.glButton(self, 22, _("Mirror Z"), (2,-4), lambda button: self.OnMirror(2))
                self.QualityButton.setExpandArrow(True)
	...
	def loadFiles(self, filenames):
		mainWindow = self.GetParent().GetParent().GetParent().GetParent()
		printWindow = mainWindow.GetName()
	...
	def OnHighQuality(self, button):

                if self._selectedObj is None:
			return
                elif not self.highQualityButton.getSelected():
                        global qualitycount
                        qualitycount = 1
                        profile.resetTempOverride()
                        print("high button pushed")
                        print(qualitycount)
                        self.setupSlice()
        ...
        def OnNormalQuality(self, button):
                if self._selectedObj is None:
			return
                elif not self.normalQualityButton.getSelected():
                        global qualitycount
                        qualitycount = 2
                        profile.resetTempOverride()
                        print("normal button pushed")
                        print(qualitycount)
                        self.setupSlice()
                        self._engine.runEngine(self._scene)
        ...
        def OnLowQuality(self, button):                
                if self._selectedObj is None:
			return
                elif not self.lowQualityButton.getSelected():
                        global qualitycount
                        qualitycount = 3
                        put = profile.setTempOverride
                        print("low button pushed")
                        print(qualitycount)
	...
	def OnToolSelect(self, button):
		...
		elif self.QualityButton.getSelected():
                        self.tool = previewTools.toolNone(self)
		else:
			self.tool = previewTools.toolNone(self)
		...
		self.highQualityButton.setHidden(not self.QualityButton.getSelected())
		self.normalQualityButton.setHidden(not self.QualityButton.getSelected())
		self.lowQualityButton.setHidden(not self.QualityButton.getSelected())
	...
	def updateToolButtons(self):
		if self._selectedObj is None:
			...
			hidden = False
		self.QualityButton.setHidden(hidden)
		...
		if hidden:
			...
			self.QualityButton.setSelected(False)
			self.OnToolSelect(0)
		...
		if machine.startswith('neuron'):
			if machine not in self._platformMesh:
				...
				if machine == 'neuron':
                                        ...
                                        self._platformMesh[machine]._drawOffset = numpy.array([0,-25,10], numpy.float32)
					#self._platformMesh[machine]._drawOffset = numpy.array([0,-37,145], numpy.float32)
				else:
					self._platformMesh[machine]._drawOffset = numpy.array([0,20,10], numpy.float32)
			...
			#For the Ultimaker 2 render the texture on the back plate to show the Ultimaker2 text.
                        if machine == 'neuron2':
                                if not hasattr(self._platformMesh[machine], 'texture'):
                                        self._platformMesh[machine].texture = openglHelpers.loadGLTexture('neuronbackplate.png')
	...
	def viewlogin(self,e):
		global login_counter
		if login_counter is None:
			login_counter=[1]
			mainWindow.neuronPanel.Show(False)
		else:
			login_counter=None
			mainWindow.neuronPanel.Show(True)
