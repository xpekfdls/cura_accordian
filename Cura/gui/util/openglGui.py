...
class glGuiPanel(glcanvas.GLCanvas):
	...
	def _drawGui(self):
		if self._glButtonsTexture is None:
			...
			self._glButtonsTexture_extended = openglHelpers.loadGLTexture('glButtons_extended.png')
...
class glButton(glGuiControl):
	def __init__(self, parent, imageID, tooltip, pos, callback, size = None, texture = 1):
		...
		self._altTooltip = ''
		self._showUnderArrow = False
		self._sideTooltip = ''
                self._texture = texture
	def setSelected(self, value):
		self._selected = value

	def setExpandArrow(self, value, direction=False):
		self._showExpandArrow = value
		self._showUnderArrow = direction
	...
	def draw(self):
		...
		pos = self._getPixelPos()
                ...
                if(self._texture == 1):
                        ...
                        glBindTexture(GL_TEXTURE_2D, self._base._glButtonsTexture_extended)
		...
		openglHelpers.glDrawTexturedQuad(pos[0]-bs*scale/2, pos[1]-bs*scale/2, bs*scale, bs*scale, self._imageID)
                if self._showExpandArrow & self._showUnderArrow:
                        if self._selected:
                                openglHelpers.glDrawTexturedQuad(pos[0]+bs*scale/2-bs*scale/4*1.2, pos[1]+bs*scale/2*0.75, bs*scale/4, bs*scale/4, 1, 2)
                        else:
                                openglHelpers.glDrawTexturedQuad(pos[0]+bs*scale/2-bs*scale/4*1.2, pos[1]+bs*scale/2*0.75, bs*scale/4, bs*scale/4, 1)
                elif self._showExpandArrow & (not self._showUnderArrow):
                        if self._selected:
		...
		elif len(self._sideTooltip) > 0:
			glPushMatrix()
			glTranslatef(pos[0], pos[1], 0)
			#glTranslatef(0, 0.6*bs, 0)
			glTranslatef(0.6*bs, -0.275*bs, 0)
			glTranslatef(0, 6, 0)
			#glTranslatef(0.6*bs*scale, 0, 0)

			for line in self._sideTooltip.split('\n'):
				glPushMatrix()
				glColor4ub(60,60,60,255)
				glTranslatef(-1, -1, 0)
				openglHelpers.glDrawStringLeft(line)
				glTranslatef(0, 2, 0)
				openglHelpers.glDrawStringLeft(line)
				glTranslatef(2, 0, 0)
				openglHelpers.glDrawStringLeft(line)
...
class glRadioButton(glButton):
	def __init__(self, parent, imageID, tooltip, pos, group, callback, texture = 1):
